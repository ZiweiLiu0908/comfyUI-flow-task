...
class ScheduledGenerationRunItem(Base):
    ...
    matched_rule_label: Mapped[str | None] = mapped_column(String(40), nullable=True)
    threshold_min_views: Mapped[int | None] = mapped_column(Integer, nullable=True)
    threshold_max_views: Mapped[int | None] = mapped_column(Integer, nullable=True)
    matched_rule_snapshot: Mapped[dict | None] = mapped_column(JSON, nullable=True)
